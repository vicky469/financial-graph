...
describe("Reprocessing Verification", () => {
  ...
  test("Should find the previously unknown company", async () => {
    ...
    expect(res.company.length).toBe(1);
    expect(res.company[0].name).toBe(TEST_NAME);
    expect(res.company[0].type).toBe(3); // Type 3 = ISSUER
  });
  ...
  test("Should find filings for this company", async () => {
    ...
    const compRes = await db.query({
      company: { $: { where: { name: TEST_NAME } } },
    });
    const companyId = compRes.company[0].id;
    ...
    expect(fileRes.filing.length).toBeGreaterThan(0);
    const filing = fileRes.filing.find((f) => f.form_type === "10-K");
  ...
  test("Should search for Santander Trust and confirm it is an issuer", async () => {
    ...
    expect(res.company.length).toBe(1);
    expect(res.company[0].name).toBe(TRUST_NAME);
    expect(res.company[0].type).toBe(3); // Type 3 = ISSUER
  });
});
