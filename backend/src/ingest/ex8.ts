...
const logger = createLogger("ingest/ex8");

// Env Config
const SEC_YEARS = process.env.SEC_YEARS!;
...
async function main() {
  logger.info(
    `🔍 Looking for 20-F filings for years: ${TARGET_YEARS.join(", ")}...`
  );
  ...
  const pendingFilings = allFilings.filter((f) => {
    ...
    return !Object.keys(f.attachments).some((k) => k.startsWith("EX-8"));
  });
  ...
  const processFiling = async (filing: Filing, index: number) => {
    ...
    try {
      ...
      // Regex for EX-8.* (Opinion re: Tax Matters)
      const regex = /<TYPE>EX-8(\S*)[\s\S]*?<FILENAME>(.*?)\n/i;
