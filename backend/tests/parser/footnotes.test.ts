...
} from "../../src/parsers/subsidiary/footnotes";

describe("extractFootnoteRefFromName", () => {
  it("extracts (n) style refs from name", () => {
    expect(extractFootnoteRefFromName("EI Freight Forwarding Co. Ltd. (12)")).toEqual(["12"]);
    expect(extractFootnoteRefFromName("Expeditors (Thailand) Ltd. (4)")).toEqual(["4"]);
  });

  it("extracts multiple refs", () => {
    expect(extractFootnoteRefFromName("Subsidiary (1)(2)(3)")).toEqual(["1", "2", "3"]);
    expect(extractFootnoteRefFromName("Company Name (1)(4)")).toEqual(["1", "4"]);
  });
...
describe("parseOwnershipWithFootnoteRef", () => {
  it("extracts ownership and ref from 100%1 pattern", () => {
    expect(parseOwnershipWithFootnoteRef("100%1")).toEqual({ ownership: 100, refs: ["1"] });
    expect(parseOwnershipWithFootnoteRef("51%2")).toEqual({ ownership: 51, refs: ["2"] });
    expect(parseOwnershipWithFootnoteRef("83.33%22")).toEqual({ ownership: 83.33, refs: ["22"] });
  });

  it("extracts ownership without ref", () => {
    expect(parseOwnershipWithFootnoteRef("100%")).toEqual({ ownership: 100, refs: [] });
    expect(parseOwnershipWithFootnoteRef("51%")).toEqual({ ownership: 51, refs: [] });
  });

  it("returns undefined for non-ownership text", () => {
    expect(parseOwnershipWithFootnoteRef("Delaware")).toEqual({ ownership: undefined, refs: [] });
    expect(parseOwnershipWithFootnoteRef("")).toEqual({ ownership: undefined, refs: [] });
  });
});
