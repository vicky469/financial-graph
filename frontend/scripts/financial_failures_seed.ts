...
//   const nodeIds = SEED_NODES.map(() => uuidv4());
...
//   // Nodes
//   const nodeOps = SEED_NODES.map((n, i) =>
//     db.tx.nodes[nodeIds[i]].update({
//       ...n,
//       createdAt: Date.now(),
//       createdBy: "Seed Script",
//     })
...
//   // Node Edges (Nodes to Events)
//   // 0: Fed -> 0: Fed Raises Rates
//   // 1: SVB -> 3: Withdrawals
//   // 3: Tech Startups -> 2: Startups Burn Cash Faster
//   const nodeEdgeData: { s: number; t: number; label?: string }[] = [
//     { s: 0, t: 0 },
//     { s: 1, t: 3 },
//     { s: 3, t: 2 },
//   ];

//   const nodeEdgeOps = nodeEdgeData.map(({ s, t, label }) =>
//     db.tx.edges[uuidv4()].update({
//       sourceId: nodeIds[s],
//       targetId: eventIds[t],
...
//   console.log(`Seeding ${eventOps.length} events, ${nodeOps.length} nodes, and connections...`);

//   await db.transact([...eventOps, ...nodeOps, ...edgeOps, ...nodeEdgeOps]);
